...
#read data
data <- read.csv(file = 'data_exercise.csv')
data <- data[c(1, 29:39, 41:51, 53:63)]
data$ï..condition[data$ï..condition == 1] <- 'Unimodal'
data$ï..condition[data$ï..condition == 2] <- 'Bimodal'
data$ï..condition <- as.factor(data$ï..condition)
data$ppt <- 1:nrow(data)
names(data)[1] <- 'distribution'

ppt.max <- nrow(data) #run analysis up to this participant

library(reshape)
data.m <- melt(data, id=c("ppt", "distribution"))
data.m$stimuli <- NaN
data.m$question <- NaN
data.u <- subset(data.m, distribution=='Unimodal')
data.b <- subset(data.m, distribution=='Bimodal')

stim.u <- c(26,92,108,120,132,144,156,168,180,196,252)
stim.b <- c(36,48,60,76,92,144,196,212,228,240,252)

for (stim.idx in 1:length(stim.u)){
  term <- paste('Q', as.character(stim.idx), sep='')  
...
data$question <- substr(data$variable, 5, 5)
names(data)[4] <- 'response'

data$question[data$question == 1] <- 'How healthy is it for the individual?'
data$question[data$question == 2] <- 'The extent of health benifits?'
data$question[data$question == 3] <- 'Suffient to avoid future problems attributed to low excercise?'
...
{
  optim(par = par, fn = sdbs_range.wrapper.ppt, ppt.data = ppt.data)
}
