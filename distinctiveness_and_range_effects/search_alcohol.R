...
#read data
data <- read.csv(file = 'data_alcohol.csv')
names(data)[5] <- 'distribution'
data <- subset(data, (distribution == 'bimodal')|(distribution == 'unimodal'))
...
resp.idx <- c(7:17, 19:29, 31:41, 43:53, 55:65, 67:77, 79:89, 91:101)
data.1 <- cbind(data.1, data[,resp.idx])
...
stim.u <- c(5, 19, 26.5, 30, 32, 33, 34, 36, 39.5, 47, 61)
stim.b <- c(5, 6, 8, 11.5, 19, 33, 47, 54.5, 58, 60, 61)

letters <- c('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K')

for (stim.idx in 1:length(stim.u)){
  data.u$stimuli[grep(pattern = letters[stim.idx], x = data.u$variable)] <- stim.u[stim.idx]
  data.b$stimuli[grep(pattern = letters[stim.idx], x = data.b$variable)] <- stim.b[stim.idx]
}
...
sdbs.search$c <- abs(sdbs.search$c)
sdbs.search$s <- abs(sdbs.search$s)
sdbs_range.search$c <- abs(sdbs_range.search$c)
sdbs_range.search$s <- abs(sdbs_range.search$s)
sdbs_range.search$w <- (1/(1+exp(-sdbs_range.search$w)))
sgems.search$c <- abs(sgems.search$c)
sgems.search$s <- abs(sgems.search$s)
