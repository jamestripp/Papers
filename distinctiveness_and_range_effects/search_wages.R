...
#read data
data <- read.csv(file = 'data_wages.csv')
ppt.max <- length(unique(data$ppt)) #run up to this ppt - controls size of the search grid
...
{
  -2 * dnorm(data, mean = prediction, sd = std, log = TRUE)
}
...
{
  fit <- by(ppt.data, ppt.data$distribution, sdbs.wrapper.data, c = par[1], s = par[2], std = par[3])
  fit <- unlist(fit)
  #print(mean(fit))
  #print(fit)
  return(sum(fit))
...
{
  optim(par = par, fn = sdbs.wrapper.ppt, ppt.data = ppt.data)#, method = "L-BFGS-B", lower = c(0.01, 0.01, 0.01, 0.01), upper = c(1000, 1000, 1, 1000))
}
...
{
  fit <- by(ppt.data, ppt.data$distribution, sdbs_range.wrapper.data, c = par[1], s = par[2], w = par[3], std = par[4])
  fit <- unlist(fit)
  #print(mean(fit))
  #print(fit)
  return(sum(fit))
...
{
  fit <- by(ppt.data, ppt.data$distribution, sgems.wrapper.data, c = par[1], s = par[2], y = par[3], std = par[4])
  fit <- unlist(fit)
  #print(mean(fit))
  #print(fit)
  return(sum(fit))
...
sgems.search$c <- (1/(1+exp(-sgems.search$c)))*100
sgems.search$s <- (1/(1+exp(-sgems.search$s)))*100
